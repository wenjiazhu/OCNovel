...
def get_sync_info_prompt(story_content: str, existing_sync_info: str = "", current_chapter: int = 0) -> str:
    """生成用于创建同步信息的提示词，仅抽取关键剧情作为前期提要"""
    return f"""根据故事进展整理相关信息，具体要求：
1. 合理细化使得相关信息逻辑完整，但不扩展不存在的设定，未尽之处可参考 [同步信息]。
2. 你在整理信息的时候，只保留对后续故事发展有参考借鉴意义的内容，如果是对后续故事不再有影响的人和事，可以不再归纳出来。
3. 仅抽取关键剧情作为前期提要，忽略次要细节。
4. 精简表达，去除一切不必要的修饰，确保信息有效的同时使用最少tokens。
5. 非常重要：你必须仅返回标准的JSON格式，不要添加任何前后缀、说明或标记。
