                ...
                with open(summary_file, 'r', encoding='utf-8') as f:
                    ...
                    if len(summaries) > 0:
                        global_summary = "\n".join([v for k, v in summaries.items() 
                                                if int(k) < chapter_idx])[-2000:]
            except Exception as e:
                logging.error(f"读取全局摘要失败: {str(e)}")
        ...
        if chapter_idx > 0:
            ...
            if os.path.exists(summary_file):
                try:
                    with open(summary_file, 'r', encoding='utf-8') as f:
                        summaries = json.load(f)
                        prev_key = str(chapter_idx)
                        if prev_key in summaries:
                            previous_summary = summaries[prev_key]
                except Exception as e:
    ...
    def _get_character_info(self, characters: Dict[str, Any], chapter_outline: Dict[str, Any]) -> str:
        ...
        for name, char in characters.items():
            ...
            if name in chapter_outline.get('characters', []):
                character_info += f"{name}: {getattr(char, 'role', '未知')}, {getattr(char, 'development_stage', '未知')}\n"
                character_info += f"- 性格: {getattr(char, 'temperament', '未知')}, 境界: {getattr(char, 'realm', '未知')}, 门派: {getattr(char, 'sect', '未知')}\n"
